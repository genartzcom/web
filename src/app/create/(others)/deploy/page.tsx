...
export default function DeployPage() {
  ...
  const handleDeploy = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/deploy`, {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error('Deploy işlemi başarısız oldu');
      }

      const data = await response.json();

      setDeployLogs(prev => [...prev, 'Deploy işlemi sürüyor...']);

      // Deploy işleminin tamamlanması için bekleyebiliriz
      setTimeout(() => {
        setDeployLogs(prev => [...prev, 'Deploy tamamlandı!']);
        setDeploying(false);
        router.push(`/collection/${data.id}`);
      }, 2000); // Gerçek uygulamada bu kısım endpoint'in deploy işlemini bitirene kadar beklemeli

    } catch (error) {
      console.error('Deploy hatası:', error);
      setDeployLogs(prev => [...prev, 'Hata oluştu: ' + (error instanceof Error ? error.message : 'Bilinmeyen hata')]);
      setDeploying(false);
    }
  };
